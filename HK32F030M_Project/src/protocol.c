...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_GRAIN_SURPLUS, DP_TYPE_VALUE},
...
void uart_transmit_output(unsigned char value)
{
	...
	extern void Uart_Send(unsigned char value);
	Uart_Send(value);       
}
 ...
 */
extern unsigned char QuickFeed_State;
extern unsigned char ManualFeed_State;

void all_data_update(void)
{
		mcu_dp_bool_update(DPID_QUICK_FEED,QuickFeed_State);        //BOOL型数据上报;
    mcu_dp_value_update(DPID_MANUAL_FEED,ManualFeed_State);     //VALUE型数据上报;
    ...
    mcu_dp_enum_update(DPID_STATUS,当前余粮状态); //枚举型数据上报;
    mcu_dp_value_update(DPID_GRAIN_SURPLUS,当前粮桶余粮); //VALUE型数据上报;
    mcu_dp_value_update(DPID_PERCENT_STATE,当前余粮百分比); //VALUE型数据上报;
    ...
    mcu_dp_string_update(DPID_COLOUR_DATA,当前彩光调节指针,当前彩光调节数据长度); //STRING型数据上报;
    mcu_dp_enum_update(DPID_UNIT_SWITCH,当前单位转换); //枚举型数据上报;
...
/*****************************************************************************
函数名称 : dp_download_grain_surplus_handle
功能描述 : 针对DPID_GRAIN_SURPLUS的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_grain_surplus_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为VALUE
    unsigned char ret;
    unsigned long grain_surplus;
    
    grain_surplus = mcu_get_dp_download_value(value,length);
    /*
    ...
    */
    ...
    ret = mcu_dp_value_update(DPID_GRAIN_SURPLUS,grain_surplus);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        break;
        case DPID_GRAIN_SURPLUS:
            //粮桶余粮处理函数
            ret = dp_download_grain_surplus_handle(value,length);
        break;
